...
if all(k in st.session_state for k in ('cv_data', 'job_description_json')):
    ...
    prompt_refinements = st.text_area(label='Prompt refinements',
                                      placeholder='Anything you want to add to the prompt',
                                      value=st.session_state.get('prompt_refinements'))
    if prompt_refinements != st.session_state.get('prompt_refinements'):
        st.session_state.prompt_refinements = prompt_refinements
